...
MemoryAllocator::MemoryAllocator(Memory _memory)
    : memory(_memory), reserved_ptr(nullptr), instance_ptr(nullptr),
      ...
      instance_total_size(0), instance_allocated_size(0), log_instance_creation(false) {
...
void MemoryAllocator::create_legion_instance(RegionInstance &inst,
                                             size_t size, const char *task_name) {
  ...
  if (log_instance_creation) {
    log_ff_mem_allocator.print("Created instance in memory_kind: %s memory_id: %llx size: %zu (capacity %lu) task_name: %s",
